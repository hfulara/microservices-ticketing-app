...
import { OrderStatus } from '@hftickets/common';
import { app } from '../../app';
...
it('returns a 204 with valid inputs', async () => {
  const userId = new mongoose.Types.ObjectId().toHexString();
  const price = Math.floor(Math.random() * 100000); // use with realistic stripe payemnt testing
  const order = Order.build({
    id: new mongoose.Types.ObjectId().toHexString(),
    userId,
    version: 0,
  ...
  // use with realistic stripe payemnt testing
